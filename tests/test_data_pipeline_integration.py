...
class TestDataGenerator:
    ...
    LOCATIONS = [
        ...
        {'lat': -33.9650, 'lon': 150.8500},  # Southwest
    ]
    WEATHER_CONDITIONS = [
    ...
    def generate_diverse_incidents(count: int = 100) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            # Vary incident type
            incident_type = TestDataGenerator.INCIDENT_TYPES[i % len(TestDataGenerator.INCIDENT_TYPES)]
            ...
            # Vary weather
            weather = TestDataGenerator.WEATHER_CONDITIONS[i % len(TestDataGenerator.WEATHER_CONDITIONS)]
            weather_with_noise = {
...
class TestDataPipelineValidation:
    ...
    def test_validate_100_diverse_incidents(self):
        ...
        assert p95_latency < 20, f"Validation p95 latency {p95_latency:.2f}ms exceeds budget of 20ms"
...
class TestSurvivalAnalysis:
    ...
    def test_impute_missing_end_datetime(self):
        ...
        for incident in new_incidents:
            ...
            assert result['end_datetime'] > incident_dict['start_datetime'], \
                "end_datetime should be after start_datetime"
        
        # Latency validation
        p95_latency = np.percentile(imputation_latencies, 95)
        assert p95_latency < 15, f"Imputation p95 latency {p95_latency:.2f}ms exceeds budget of 15ms"
...
class TestDataPipelineIntegration:
    ...
    def test_complete_pipeline_100_incidents(self, mock_embedding_model):
        ...
        **Validates: Requirements 1.3, 2.1, 3.1, 4.3, 11.1**
        ...
        fit_incidents_df = pd.DataFrame([
            self._incident_to_dict(inc) for inc in incidents[:50]
        ])
        ...
        # Assertions
        assert valid_count >= 80, f"Expected ≥80 valid incidents, got {valid_count}"
        assert complete_vectors_count >= 80, \
        ...
        assert pipeline_p95 < 50, \
            f"Total pipeline p95 {pipeline_p95:.2f}ms exceeds budget of 50ms"
    ...
    def test_pipeline_output_feature_vector_completeness(self):
        ...
        **Validates: Requirements 4.3, 11.1**
        ...
        assert len(feature_vectors) >= 90, f"Expected ≥90 feature vectors, got {len(feature_vectors)}"
...
class TestPipelinePerformance:
    ...
    def test_throughput_100_incidents_per_second(self):
        ...
        **Validates: Requirement 11.1**
        ...
        fit_df = pd.DataFrame([
            {'incident_type': 'accident', 'location_lat': -33.8688, 'location_lon': 151.2093,
             'hour_of_day': 14, 'day_of_week': 0, 'temperature': 22, 'precipitation': 0,
             'wind_speed': 5, 'severity_initial': 50}
        ])
