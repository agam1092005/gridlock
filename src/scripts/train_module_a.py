...
def prepare_features(df):
    ...
    df['priority_numeric'] = df.get('priority', 'medium').str.lower().map({'high': 2, 'medium': 1, 'low': 0}).fillna(1)
    ...
    df['is_event'] = cause.isin(['public_event', 'procession', 'protest', 'vip_movement', 'planned']).astype(int)
    
    # Extract veh_type and corridor
    veh_type = df.get('veh_type', pd.Series(['']*len(df), index=df.index)).astype(str).str.lower()
    df['is_heavy_vehicle'] = veh_type.isin(['heavy_vehicle', 'truck', 'bmtc_bus', 'ksrtc_bus']).astype(int)
...
def main():
    ...
    if not os.path.exists(os.path.join(output_dir, "train.pkl")):
        ...
        # Vehicle Impact
        veh_type = df.get('veh_type', pd.Series(['']*len(df), index=df.index)).astype(str).str.lower()
        severity += veh_type.isin(['heavy_vehicle', 'truck', 'bmtc_bus', 'ksrtc_bus']).astype(int) * 20
        ...
        # Cause Impact
        cause = df.get('event_cause', pd.Series(['']*len(df), index=df.index)).astype(str).str.lower()
        severity += cause.isin(['political_rally', 'water_logging', 'construction', 'accident']).astype(int) * 15
