...
class AuditLogger:
    ...
    def log_validation_batch(
        ...
        return self.log_operation(operation="validation_batch", status=status, details=details)
    ...
    def log_embedding_operation(
        ...
        return self.log_operation(
            operation="embedding_generation", status=status, details=details, error_message=error
        )
    ...
    def log_duplicate_detection(
        ...
        return self.log_operation(
            operation="duplicate_detection", status=status, incident_id=incident_id, details=details
        )
    ...
    def export_entries(self, format: str = "json") -> str:
        ...
        if format == "json":
            return json.dumps(
                [e.model_dump(mode="json") for e in self.entries], indent=2, default=str
            )
        elif format == "csv":
            # Simple CSV format
...
class AuditContext:
    ...
    def __init__(
        self, audit_logger: AuditLogger, operation: str, incident_id: Optional[str] = None
    ):
        """
