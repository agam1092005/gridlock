...
class SurvivalAnalyzer:
    ...
    COX_FEATURES = ['location_grid_x', 'location_grid_y', 'weather_temp', 'hour_of_day', 'is_rush_hour']
    
    # Cache configuration
    DEFAULT_CACHE_TTL_DAYS = 7
    CACHE_KEY_PREFIX = "survival_model"
    
    def __init__(self, redis_client: Optional[redis.Redis] = None, cache_ttl_days: int = DEFAULT_CACHE_TTL_DAYS):
        """
    ...
    def fit_models(self, historical_incidents_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"After filtering: {len(df)} valid samples, {df['event_observed'].sum()} events observed, "
            ...
            for incident_type in df['incident_type'].unique():
                ...
                if num_samples >= self.MIN_SAMPLES_FOR_STRATIFIED_KM:
                    ...
                    logger.info(f"Skipping KM for {incident_type}: only {num_samples} samples (threshold: {self.MIN_SAMPLES_FOR_STRATIFIED_KM})")
            ...
            if cox_features_available:
                try:
                    ...
                    if len(cox_data_clean) > 0:
                        ...
                        cox_data_clean = cox_data_clean.rename(columns={
                            'duration_min': 'T',
                            'event_observed': 'E'
                        })
                ...
                logger.warning(f"Missing Cox features: {[f for f in self.COX_FEATURES if f not in df.columns]}")
                self.cox_model = None
            
            # Update metadata
            self.model_metadata['fitted_at'] = datetime.utcnow().isoformat()
    ...
    def impute_end_datetime(
        ...
        try:
            start_dt = pd.to_datetime(incident.get('start_datetime'))
    ...
    def _cache_models(self) -> bool:
        ...
        try:
            ...
            self.redis_client.setex(
                f"{cache_key}:km_curves",
                self.cache_ttl_seconds,
                pickle.dumps(km_data)
            )
            self.redis_client.setex(
                f"{cache_key}:cox_model",
                self.cache_ttl_seconds,
                cox_data or b''
            )
            self.redis_client.setex(
                f"{cache_key}:population_km",
                self.cache_ttl_seconds,
                population_km_data or b''
            )
