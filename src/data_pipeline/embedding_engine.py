...
class EmbeddingEngine:
    ...
    def embed(self, texts: List[str], normalize: bool = True) -> np.ndarray:
        ...
        for i in range(0, len(texts), batch_size):
            ...
            batch_embeddings = (
                self._mean_pool(outputs.last_hidden_state, encoded["attention_mask"]).cpu().numpy()
            )
...
class EmbeddingBatcher:
    ...
    async def add_and_wait(
        ...
        try:
            embedding = await asyncio.wait_for(future, timeout=timeout_ms / 1000.0)
    ...
    async def flush(self):
        ...
        logger.debug(
            f"Flushing embedding batch with {len(texts)} texts", extra={"batch_size": len(texts)}
        )
            ...
            for idx, text, embedding in zip(indices_to_embed, texts_to_embed, new_embeddings):
                ...
                if self.cache:
                    try:
                        ...
                        self.cache.setex(cache_key, self.cache_ttl_seconds, embedding_bytes)
                    except Exception as e:
            ...
            if not future.done():
                ...
                if embedding is not None:
                    ...
                    future.set_exception(
                        RuntimeError(f"Embedding not found for text at index {idx}")
                    )
