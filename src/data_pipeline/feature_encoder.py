...
class FeatureEncoder:
    ...
    def fit(self, incidents_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for feature in self.categorical_features:
            if feature in df.columns:
                self.feature_modes[feature] = (
                    df[feature].mode()[0] if not df[feature].mode().empty else "unknown"
                )
                missing_pct = (df[feature].isna().sum() / len(df)) * 100
                self.missing_percentages[feature] = missing_pct
    ...
    def _generate_derived_features(self, incident: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Ensure optional fields are present to maintain feature dictionary key consistency
        if "end_datetime" not in derived:
            derived["end_datetime"] = None
        if "severity_initial" not in derived:
            derived["severity_initial"] = None
    ...
    def encode(self, incident: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if numerical_cols:
            ...
            df_standardized = pd.DataFrame(
                self.scaler.transform(df_numerical), columns=numerical_cols
            )
        ...
        if categorical_cols:
            ...
            try:
                ...
                # Ensure it's a dense numpy array; use getattr to avoid type checker errors
                if not isinstance(encoded_categorical, np.ndarray):
                    encoded_categorical = getattr(encoded_categorical, "toarray")()
                ...
                logger.debug(
                    f"Categorical encoding partial (may be expected on unfitted encoder): {e}"
                )
...
class FeatureStore:
    ...
    def add_feature_vector(
        self, incident_id: str, features: Dict[str, Any], metadata: Optional[Dict[str, Any]] = None
    ) -> None:
        """
