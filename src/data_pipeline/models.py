...
class IncidentInput(BaseModel):
    ...
    incident_id: Optional[str] = Field(
        None,
        description="Unique incident identifier (UUID format)"
    )
    ...
    timestamp: datetime = Field(
        ...,
        description="Incident timestamp in ISO 8601 format"
    )
    description: str = Field(
        ...,
        min_length=10,
        max_length=5000,
        description="Incident description (10-5000 characters)"
    )
    ...
    severity_initial: Optional[float] = Field(
        ...
        ge=0,
        le=100,
        description="Initial severity estimate (0-100)"
    )
    end_datetime: Optional[datetime] = Field(
        None,
        description="Incident end time (typically null for ongoing incidents)"
    )
    weather: Optional[WeatherData] = Field(
        None,
        description="Weather conditions at incident location"
    )
    is_ongoing: Optional[bool] = Field(
        True,
        description="Whether incident is still ongoing"
    )
    num_lanes_blocked: Optional[int] = Field(
        None,
        ge=0,
        description="Number of lanes blocked"
    )
...
class ValidationError(BaseModel):
    ...
    value: Optional[str] = Field(
        None,
        description="The value that failed validation (sanitized)"
    )
...
class ValidationResult(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={
        "example": {
            ...
            "warnings": [
                "Weather data is missing, using defaults"
            ],
    ...
    duplicate_incident_ids: List[str] = Field(
        default_factory=list,
        description="IDs of duplicate incidents detected"
    )
...
class AuditLogEntry(BaseModel):
    ...
    status: str = Field(
        ...,
        description="Operation status (success, failure, partial)"
    )
