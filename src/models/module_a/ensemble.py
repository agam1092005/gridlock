...
class ModuleAEnsemble:
    ...
    def calibrate_on_validation(self, val_predictions, val_ground_truth):
        ...
        try:
            ...
            self.duration_calibrator.fit(
                val_predictions[:, 3],  # Raw confidence from model
                1 - duration_normalized_errors
            )
    ...
    def predict(self, structured_features, embedding, historical_seq=None, incident_type="unknown"):
        ...
        return {
            ...
            "severity_ci": [float(np.clip(lgb_sev_pred['ci_lower'][0], 0, 100)), float(np.clip(lgb_sev_pred['ci_upper'][0], 0, 100))],
            "duration_estimate": float(max(0, calib_dur)),
            "duration_ci": [float(max(0, lgb_dur_pred['ci_lower'][0])), float(max(0, lgb_dur_pred['ci_upper'][0]))]
        }
