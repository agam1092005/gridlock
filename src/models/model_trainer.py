...
class ModelTrainer:
    ...
    def submit_training_job(self, component: str, hyperparameters: dict, dataset_metadata: dict):
        ...
        mock_metrics = {"RMSE": 4.2, "MAE": 3.1, "R2": 0.88, "C-index": 0.85}
        ...
        if not active:
            ...
            if mock_metrics["R2"] > old_r2 * 1.05:  # >5% improvement
                logger.info(
                    f"New model improves R2 ({old_r2} -> {mock_metrics['R2']}). Marking ready for evaluation."
                )
                promote = True
            else:
        ...
        return {"version_id": version_id, "metrics": mock_metrics, "promoted": promote}
