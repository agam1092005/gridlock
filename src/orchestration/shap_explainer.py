...
class SHAPExplainer:
    def __init__(self, mod_a_sev=None, mod_a_dur=None):
        ...
        self.feature_names = [
            "latitude", "longitude", "priority", "is_construction", 
        ...
        if self.sev_explainer and model_inputs is not None:
            try:
                ...
                if hasattr(self.sev_explainer, 'expected_value'):
                    ...
                    base_value = expected[0] if isinstance(expected, (list, np.ndarray)) else expected
                ...
                for i, name in enumerate(self.feature_names):
                    severity_features.append({
                        "name": name,
                        "shap_value": float(structured_shap[i])
                    })
                
                severity_features.append({
                    "name": "text_description_impact",
                    "shap_value": float(embedding_impact)
                })
                
                # Sort by absolute impact
                severity_features = sorted(severity_features, key=lambda x: abs(x["shap_value"]), reverse=True)[:5]
        ...
        if self.dur_explainer and model_inputs is not None:
            try:
                ...
                if hasattr(self.dur_explainer, 'expected_value'):
                    ...
                    dur_base_value = expected[0] if isinstance(expected, (list, np.ndarray)) else expected
                ...
                duration_features = sorted(duration_features, key=lambda x: abs(x["shap_value"]), reverse=True)[:5]
