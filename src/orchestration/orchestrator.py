...
class PredictionOrchestrator:
    ...
    async def _call_module_a(self, context):
        ...
        def run_inference():
            try:
                ...
                if is_missing_end:
                    from .survival_model import SurvivalModelSingleton, INCIDENT_TYPE_MAP, PRIORITY_MAP, HISTORICAL_MEDIANS
                    ...
                    if inc_type_str not in INCIDENT_TYPE_MAP or priority_str not in PRIORITY_MAP:
                        duration_estimate = HISTORICAL_MEDIANS.get(inc_type_str, HISTORICAL_MEDIANS["generic_fallback"])
                        logger.warning(f"Novel feature encountered (type: {inc_type_str}, priority: {priority_str}). Falling back to historical median: {duration_estimate}")
                    else:
                        ...
                        try:
                            ...
                            if cph is not None:
                                ...
                                duration_estimate = float(cph.predict_expectation(covariates_df).values[0])
                                logger.info(f"Calculated survival duration expectation: {duration_estimate:.2f} mins")
                            else:
                                duration_estimate = HISTORICAL_MEDIANS.get(inc_type_str, HISTORICAL_MEDIANS["generic_fallback"])
                                logger.warning(f"Cox model not loaded, using fallback median for {inc_type_str}: {duration_estimate}")
                        except Exception as e:
                            duration_estimate = HISTORICAL_MEDIANS.get(inc_type_str, HISTORICAL_MEDIANS["generic_fallback"])
                            logger.error(f"Survival duration expectation failed, falling back to median: {e}")
                else:
    ...
    async def run_pipeline(self, context) -> PredictionResponse:
        ...
        t0 = time.time()
        try:
            res_a = await asyncio.wait_for(self._call_module_a(context), timeout=1.0) # slightly relaxed timeout for embedding engine lookup
        ...
        predictions_merged = {
            "module_a": res_a,
            "module_b": res_b if isinstance(res_b, dict) else {"geojson": {"type": "FeatureCollection", "features": []}}
        }
        ...
        asyncio.create_task(
            ws_manager.broadcast({
                ...
                "module_b_geojson": res_b.get("geojson") if isinstance(res_b, dict) else None,
                "explanations": res_shap
            })
        )
