...
class BackgroundWorker:
    ...
    def process_incident(self, incident_data):
        ...
        try:
            ...
            incident_data["severity_ci"] = [
                float(np.clip(sev_pred.get("ci_lower", [0])[0], 0, 100)),
                float(np.clip(sev_pred.get("ci_upper", [100])[0], 0, 100)),
            ]
            ...
            if is_missing_end:
                ...
                from ..orchestration.survival_model import (
                ...
                priority_str = str(
                    metadata.get("priority") or incident_data.get("priority") or "low"
                ).lower()

                if inc_type_str not in INCIDENT_TYPE_MAP or priority_str not in PRIORITY_MAP:
                    fallback_median = HISTORICAL_MEDIANS.get(
                    ...
                    logger.warning(
                        f"Novel feature encountered in worker (type: {inc_type_str}, priority: {priority_str}). Falling back to historical median: {fallback_median}"
                    )
                else:
                    ...
                    try:
                        ...
                        if cph is not None:
                            ...
                            logger.info(
                                f"Calculated survival duration expectation in worker: {duration_est:.2f} mins"
                            )
                        else:
                            fallback_median = HISTORICAL_MEDIANS.get(
                                inc_type_str, HISTORICAL_MEDIANS["generic_fallback"]
                        ...
                        logger.error(
                            f"Survival duration expectation failed in worker, falling back to median: {e}"
                        )
            else:
                duration_est = float(max(0, np.expm1(raw_est)))
                incident_data["duration_estimate"] = duration_est
                incident_data["duration_ci"] = [
                    float(max(0, np.expm1(dur_pred.get("ci_lower", [0])[0]))),
                    float(max(0, np.expm1(dur_pred.get("ci_upper", [100])[0]))),
                ]

            incident_data["predicted_by_ai"] = True

            # Compute real SHAP explanations
            incident_data["explanations"] = self.shap_explainer.compute_explanations(
                incident_data, X_input
            )
        ...
        try:
            ...
            incident_data["latest_news"] = self.news_fetcher.fetch_for_incident(
                incident_data, limit=5
            )
            incident_data["active_news_alerts"] = active_news

            if active_news:
                # Dynamically boost AI severity and duration based on real-world news!
                incident_data["severity_score"] = float(
                    np.clip(incident_data.get("severity_score", 50.0) + 15.0, 0, 100)
                )
                incident_data["duration_estimate"] = (
                    incident_data.get("duration_estimate", 30.0) * 1.5
                )
        except Exception as e:
        ...
        message = {"type": "incident_update", **incident_data}
        return message
