...
class EnvironmentValidator:
    ...
    def validate(self) -> bool:
        ...
        for var in optional_vars:
            if not os.getenv(var):
                self.warnings.append(f"Optional environment variable '{var}' not set, using default")
            else:
    ...
    def _validate_database_url(self):
        ...
        if db_url:
            ...
            if '@' not in db_url:
                self.errors.append("DATABASE_URL must contain credentials: postgresql://user:pass@host:port/db")
